...
public class TeacherController {
    ...
    /**
     * getTeacherIntroduceData 前端获取老师个人简历数据请求服务
     *
     * @param dataRequest 从前端获取 teacherId 查询老师信息的主键 teacher_id
    ...
    public DataResponse getTeacherIntroduceData(@Valid @RequestBody DataRequest dataRequest) {
        ...
        Optional<Teacher> sOp = teacherRepository.findByPersonNum(username);  // 查询获得 Student对象
        if (!sOp.isPresent())
            return CommonMethod.getReturnMessageError("老师不存在！");
        Teacher t = sOp.get();
        Map info = teacherService.getMapFromTeacher(t);  // 查询老师信息Map对象
    ...
    public List getTeacherFeeList(Integer teacherId) {
        List<Fee> sList = feeRepository.findListByStudent(teacherId);  // 查询某个学生消费记录集合
